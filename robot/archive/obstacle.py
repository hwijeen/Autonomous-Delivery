...
def obstacle_detection(image):
    ...
    middle = image[80:160, :]
    bottom = image[160:, :]
    middle_lines = find_line(middle)
    bottom_lines = find_line(bottom)
    ...
    if middle_lines is not None:
        for line in middle_lines:
            x1, y1, x2, y2 = line[0]
            mid_angle = np.arctan((y2 - y1) / (x1 - x2)) * 180.0 / np.pi
            # our angle is a deviation from the y axis
            if mid_angle < 0:
                mid_angle = -90.0 - mid_angle
            else:
                mid_angle = 90.0 - mid_angle
            cv2.line(image, (x1, y1 + 80), (x2, y2 + 80), (255, 0, 0), 3)

            if mid_angle < -80.0 or mid_angle > 80.0:
                stop_flag = True
        ...
        if max(angles) > 70.0 and min(angles) < 10.0:
            if np.abs(tops[np.argmax(angles)] - tops[np.argmin(angles)]) < 10 \
                ...
                if max(tops) - min(bots) < 0:
                    ...
                    kit.continuous_servo[0].throttle = 0
                    kit.continuous_servo[1].throttle = 0
                    destconn.send(pickle.dumps(int(3)))
...
def vision():
    ...
    while cap.isOpened():
        ...
        if mode_flag is False:
            ...
            lines = find_line(hsv[160:, :])
            left, right = line_tracking(lines)
            stop_x, stop_y = sign_detecting(hsv)
            addr_x, addr_y = address_detecting(hsv)
            image = obstacle_detection(hsv)
        ...
        # print("Elapsed time for one frame is %.4f" % (time.time() - start))
        time.sleep(0.01)
